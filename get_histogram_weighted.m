function [MI_hist, MI_prob] = get_histogram_weighted(points_gray, points_refc, weight_tmp, nbins)
...
%     tic
    ...
    [~, ~, bin_idx_gray] = histcounts(points_gray, edge_gray);
    [~, ~, bin_idx_refc] = histcounts(points_refc, edge_refc);
    % weighted 1d histogram
    hist_gray = accumarray(bin_idx_gray, weight_tmp, [nbins,1]);
    hist_refc = accumarray(bin_idx_refc, weight_tmp, [nbins,1]);
%     [hist_gray, ~] = histwv(points_gray, weight_tmp, min(points_gray), max(points_gray), nbins);
%     [hist_refc, ~] = histwv(points_refc, weight_tmp, min(points_refc), max(points_refc), nbins);
    
%     hist_joint = hist2w([points_gray, points_refc], weight_tmp, edge_gray, edge_refc);
...
%     edge_gray_refc = linspace(0.5, nbins*nbins+0.5, nbins*nbins+1);
    ...
    hist_joint = accumarray(gray_refc, weight_tmp, [nbins*nbins, 1]);
