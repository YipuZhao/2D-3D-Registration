...
%         prob_gray = conv (prob_gray, gaussFilter, 'same');
        prob_gray = gather(imgaussfilt(prob_gray, [1, sigma_gray]));
        prob_refc = gather(imgaussfilt(prob_refc, [1, sigma_refc]));
%         toc
%         disp('2D------');
%         tic
        prob_joint = gather(imgaussfilt(prob_joint, [sigma_refc, sigma_gray]));
%         tocR
    end
