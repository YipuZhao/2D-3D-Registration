function [xk] = gradient_descent_search_s2 (x0_hl, img_list, points, para, w)
%%
global res_dir
global step_num
...
core_bias_multi = [-1, 1]; % f(x-2dx), f(x-dx), f(x+dx), f(x+2dx)
for index = 1 : MAX_ITER
    ...
    delF_delPara = (v(:,2) - v(:,1))'./(2*deltaPara);
    ...
    disp(sprintf('k = %d, df = %0.6f, f = (%0.6f, %0.6f)\n', [(1:6)', delF_delPara', v]'));
    %     toc
        
    %% plot the intermediate result
    [~, t, R, inv_R, err_val] = addError(para.err_list{1}, xk, step_num+1, step_num);
    [u, v, points_id] = imageProjection_upd(points, img_list{1}, para, t, R, inv_R);
    % for kitti data
    if strcmp(para.camera_type, 'kitti')
        [u, v, points_id] = occludedPointsRemoval(u, v, points_id, img_list{1}, points);
        % for GT data
    else
        [u, v, points_id] = occludedPointsRemoval_upd(u, v, points_id, img_list{1}, points);
    end
    figure;
    imagesc(img_list{1}.image);
    hold on
    scatter(u, v, ones(1,length(u)), points(points_id, 5), 'filled');
    colormap parula
    set(gcf,'PaperUnits','inches','PaperPosition',[0 0 12 9])
    saveas(gcf, [res_dir '/Opt_iter_' num2str(index) '.png']);
    close all;
