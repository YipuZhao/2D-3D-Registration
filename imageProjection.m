function [u, v, points_id] = imageProjection(points, image, para, t, R, inv_R)
...
elseif strcmp(para.camera_type, 'kitti')
    points_img_idx = points_world(1,:)>=5;% & points_world(3,:)<-1.5 & points_world(3,:)>-2.3; % in front of cam
    points_world = points_world(:,points_img_idx);
    points_id = points_id(points_img_idx);
    Tr = [R t;0 0 0 1];
    points_world(4, :) = 1;
